...
KAGENTI_ADMIN_USER="${KAGENTI_ADMIN_USER:-admin}"
KAGENTI_ADMIN_PASSWORD="${KAGENTI_ADMIN_PASSWORD:-admin}"
...
get_client_id() {
  ...
  kc_api GET "/admin/realms/${2:-$REALM}/clients?clientId=$1" | python3 -c "import sys,json; clients=json.load(sys.stdin); print(clients[0]['id'] if clients else '')"
}
...
TOKEN_RESPONSE=$(curl -s -X POST \
  ...
  -d "grant_type=password&client_id=admin-cli&username=${KAGENTI_ADMIN_USER}&password=${KAGENTI_ADMIN_PASSWORD}")
...
# ─── Create/Update kagenti-cli Client ───────────────────────────────────────────

echo "🧩 Configuring Kagenti public client 'kagenti-cli'..."
CID=$(get_client_id "kagenti-cli" "master")
if [ -z "$CID" ]; then
  kc_api POST "/admin/realms/master/clients" -d "{
    \"clientId\": \"kagenti-cli\",
    \"enabled\": true,
    \"publicClient\": true,
    \"standardFlowEnabled\": true,
    \"directAccessGrantsEnabled\": true,
    \"redirectUris\": [\"http://kagenti.localtest.me:8080/*\"],
    \"webOrigins\": [\"*\"]
  }"
  echo "  Kagenti client created."
else
  kc_api PUT "/admin/realms/master/clients/${CID}" -d "{
    \"id\": \"${CID}\",
    \"clientId\": \"kagenti-cli\",
    \"enabled\": true,
    \"publicClient\": true,
  ...
  echo "  Kagenti-cli client updated."
fi
